#!/usr/bin/env python3
"""
Improved shared utilities for research notes.

This module demonstrates the recommended improvements for the research notes skill.
"""
        ...
        if config_file.exists():
            ...
            try:
                with open(config_file) as f:
                    ...
                    if config and ('notion' in config or 'database' in config):
                        return current
            except:
                pass
...
def list_all_projects(projects_dir: Path) -> list:
    """
    ...
    for project_dir in projects_dir.iterdir():
        ...
        try:
            ...
            if 'title' in metadata:
                projects.append(metadata['title'])
        except:
            continue
...
if __name__ == '__main__':
    print("Testing improved utilities...")
    print()
    ...
    for test in test_cases:
        result = slugify(test)
        safe = '..' not in result and '/' not in result and len(result) <= 100
        status = "✓" if safe else "✗"
        print(f"{status} {test[:50]!r:50} -> {result!r}")
    
    print()
    print("Title Validation Tests:")
    print("-" * 60)
    for test in test_cases[:5]:
        valid, msg = validate_title(test)
        status = "✓" if valid else "✗"
        print(f"{status} {test!r:40} {msg if msg else 'Valid'}")
